...
3. **Simple Port Scanner**
   ...
   - Asynchronous architecture (`asyncio`) for blazing speed
   - Real-time scan statistics

4. **Network Information Gathering Wizard**
   - Asynchronous monitoring of active network interfaces
   - Generator-based historical tracking for long-term trends
   - Detailed JSON reporting and connectivity checks
...
- **Performance Optimization**:
  - Asynchronous architecture (`asyncio`) for non-blocking operations
  - Generator-based parsing (`yields`) for memory efficient data handling
  - Process/Thread Pool offloading for synchronous blocking calls
  - Robust exception handling and Pythonic dunder methods

## 🚀 Usage

### Network Analysis
```bash
# Network Connection Analyzer (Live monitoring)
python "Network Connection Analyzer/network_connection_analyzer.py" -m 60 -v

# Network Information Gathering Wizard (Generate report)
python "Network Information Gathering Wizard/network_analyzer.py" -r
```

### Port Scanning
```bash
# Industrial scanner
python "Network Port Scanner/port_scanner.py" 192.168.1.0/24 1-1024 -p tcp,udp

# Simple TCP scanner
python "Simple Network Port Scanner/port_scanner.py" 10.0.0.1 20 100 -b -j 200
```
